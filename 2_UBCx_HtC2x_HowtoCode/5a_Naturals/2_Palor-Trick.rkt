...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 2_Palor-Trick) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; PROBLEM:
;
...
(define (SUB1 n) (rest n))     ;NATURAL[>0] -> NATURAL (rest cannot be empty)
...
#;; This is the real natural numbers,looks same
 (define (fn-for-natural n)
  (cond [(zero? n) (...)]
        [else
         (...n ;
...
(define (ADD a b)     ;adding from a to b
  (cond [(ZERO? b) a]
        [else
         (ADD (ADD1 a) (SUB1 b))]))
...
(check-expect (SUBTRACT N5 N3) N2)
