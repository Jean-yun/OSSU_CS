...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Quiz_design-quiz-pyramid-sink-solid) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)

;; ======================================================================
;; Constants
(define C (circle 12 "solid" "brown"))
...
(check-expect (pyramid 0 C) empty-image)
(check-expect (pyramid 1 C) C)
(check-expect (pyramid 2 C) (above C
                                   (beside C C empty-image)))

(check-expect (pyramid 3 C) (above C
                                   (beside C C)
                                   (beside C C C empty-image)))
...
;---helper function
(check-expect (row-images 2 C) (beside C C empty-image))
(check-expect (row-images 3 C) (beside C C C empty-image))

(define (row-images n i)
  (cond [(zero? n) empty-image]
        [else
         (beside i (row-images (sub1 n) i))]))
...
;-----------helper------------
(check-expect (sink-by-one "solid" (cons "bubble" empty))
              (cons "bubble" (cons "solid" empty)))

(check-expect (sink-by-one "solid" (cons "bubble" (cons "bubble" empty)))
              (cons "bubble" (cons "solid" (cons "bubble" empty))))

(check-expect (sink-by-one "solid" (cons "solid" (cons "bubble" empty)))
