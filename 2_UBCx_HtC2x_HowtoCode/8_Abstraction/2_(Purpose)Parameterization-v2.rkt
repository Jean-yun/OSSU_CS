...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname 2_Parameterization-v2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (contains? "UBC" (cons "UBC" empty)) true)
(check-expect (contains? "UBC" (cons "McGill" (cons "UBC" empty))) true)
(check-expect (contains? "Toronto" (cons "UBC" empty)) false)
...
;;<TESTS>
;;<Purpose Statement>
;;given pred(predicate) and list, produce list that fulfills pred
(check-expect (filter2 positive? empty) empty)
(check-expect (filter2 positive? (list 3 2)) (list 3 2))
(check-expect (filter2 negative? (list 3 -5 1 -7)) (list -5 -7))
(check-expect (filter2 zero? (list 4 9 3 0)) (list 0))
