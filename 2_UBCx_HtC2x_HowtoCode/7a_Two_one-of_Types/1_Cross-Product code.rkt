...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname |1_Cross-Product code|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (prefix=? (list "x") empty) false)   ;lsta > lstb
(check-expect (prefix=? empty (list "x")) true)    ;lsta < lstb
...
;(define (prefix=? lsta lstb) false) ;stubΩ4
...
(define (prefix=? lsta lstb)
  (cond [(empty? lsta) true]
        [(empty? lstb) false]
        [else (and (string=? (first lsta) (first lstb))
